...
test_dataset = DocVQADataset(data['validation'])

# Create a subset of the dataset
subset_size = int(0.01 * len(test_dataset))  # 10% of the dataset
indices = random.sample(range(len(test_dataset)), subset_size)
subset_dataset = Subset(test_dataset, indices)

test_loader = DataLoader(subset_dataset, batch_size=batch_size, collate_fn=collate_fn, num_workers=num_workers, prefetch_factor=prefetch_factor)
...
print(f"Average Normalized Levenshtein Similarity: {average_similarity}")
